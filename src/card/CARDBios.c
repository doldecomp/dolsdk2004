...
void __CARDExtHandler(s32 chan, OSContext *context) {
    ...
    if (card->attached)
    {
        ASSERTLINE(0xEF, card->txCallback == 0);
        card->attached = FALSE;
...
void __CARDExiHandler(s32 chan, OSContext *context) {
    ...
    ASSERTLINE(0x11B, 0 <= chan && chan < 2);
...
void __CARDTxHandler(s32 chan, OSContext *context) {
    ...
    ASSERTLINE(0x16D, 0 <= chan && chan < 2);
...
void __CARDUnlockedHandler(s32 chan, OSContext *context) {
    ...
    ASSERTLINE(0x19c, 0 <= chan && chan < 2);
...
s32 __CARDEnableInterrupt(s32 chan, BOOL enable) {
    ...
    ASSERTLINE(0x1AF, 0 <= chan && chan < 2);
...
s32 __CARDReadStatus(s32 chan, u8 *status) {
    ...
    ASSERTLINE(0x1C2, 0 <= chan && chan < 2);
...
int __CARDReadVendorID(s32 chan, u16 *id) {
    ...
    ASSERTLINE(0x1D7, 0 <= chan && chan < 2);
...
s32 __CARDClearStatus(s32 chan) {
    ...
    ASSERTLINE(0x1EC, 0 <= chan && chan < 2);
...
long __CARDSleep(long chan) {
    ...
    ASSERTLINE(0x1FF, 0 <= chan && chan < 2);
...
long __CARDWakeup(long chan) {
    ...
    ASSERTLINE(0x212, 0 <= chan && chan < 2);
    if (!EXISelect(chan, 0, CARDFreq)) {
        return CARD_RESULT_NOCARD;
    }
    cmd = 0x87000000;
...
static void TimeoutHandler(OSAlarm *alarm, OSContext *context) {
    ...
    ASSERTLINE(0x242, 0 <= chan && chan < 2);
...
static s32 Retry(s32 chan) {
    ...
    ASSERTLINE(0x28E, 0 <= chan && chan < 2);
...
static void UnlockedCallback(s32 chan, s32 result) {
    ...
    ASSERTLINE(0x2CE, 0 <= chan && chan < 2);
...
static s32 __CARDStart(s32 chan, CARDCallback txCallback, CARDCallback exiCallback) {
    ...
    ASSERTLINE(0x310, 0 <= chan && chan < 2);
...
s32 __CARDReadSegment(s32 chan, CARDCallback callback) {
    ...
    ASSERTLINE(0x34E, 0 <= chan && chan < 2);

    card = &__CARDBlock[chan];
    ASSERTLINE(0x350, card->addr % CARD_SEG_SIZE == 0);
    ASSERTLINE(0x351, card->addr < (u32) card->size * 1024 * 1024 / 8);
    card->cmd[0] = 0x52;
...
s32 __CARDWritePage(s32 chan, CARDCallback callback) {
    ...
    ASSERTLINE(0x387, 0 <= chan && chan < 2);

    card = &__CARDBlock[chan];
    ASSERTLINE(0x389, card->addr % card->pageSize == 0);
    ASSERTLINE(0x38A, card->addr < (u32) card->size * 1024 * 1024 / 8);
    card->cmd[0] = 0xF2;
...
long __CARDErase(long chan, void (* callback)(long, long)) {
    ...
    ASSERTLINE(0x3C2, 0 <= chan && chan < 2);
...
s32 __CARDEraseSector(s32 chan, u32 addr, CARDCallback callback) {
    ...
    ASSERTLINE(0x3F2, 0 <= chan && chan < 2);

    card = &__CARDBlock[chan];
    ASSERTLINE(0x3F4, addr % card->sectorSize == 0);
    ASSERTLINE(0x3F5, addr < (u32) card->size * 1024 * 1024 / 8);
...
const DVDDiskID* CARDGetDiskID(s32 chan) {
    ASSERTLINE(0x490, 0 <= chan && chan < 2);
...
s32 CARDSetDiskID(s32 chan, const DVDDiskID* diskID) {
    ...
    card = &__CARDBlock[chan];
    ASSERTLINE(0x4A5, 0 <= chan && chan < 2);
...
s32 __CARDPutControlBlock(CARDControl *card, s32 result) {
    ...
    ASSERTLINE(0x4EB, result != CARD_RESULT_BUSY);
...
s32 CARDGetResultCode(s32 chan) {
    ...
    ASSERTLINE(0x50C, 0 <= chan && chan < 2);
